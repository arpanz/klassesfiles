...
def validate(data, args, elective_mode=False):
    ...
    and skips the unresolved-placeholder check (PE-3 is never in an electives file).
    """
    errors = []
        ...
        for day, slots in days.items():
            ...
            for slot, info in slots.items():
                ...
                if not elective_mode:
                    ...
                    # Pipes only flagged when --pe3 was used (non-pe3 files can't have
                    # unresolved pipe-options by definition).
